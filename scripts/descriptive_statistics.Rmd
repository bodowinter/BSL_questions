...
```{r}
df <- df %>%
  mutate(mouth_gesture_question_01 = ifelse(mouth_gesture_question == 'YES', 1, 0),
  ...
  mutate(strategy_count = mouth_gesture_question_01 +
           mouth_gesture_question_01 +
           manual_01 + nonmanual_01 +
           any_eye_brow_01 + 
           any_head_movement_01) %>% 
  
  # get rid of _01 helper variables:
  select(-mouth_gesture_question_01, -mouth_gesture_question_01,
         -manual_01, -nonmanual_01,
...
strategy_p <- strat_tab %>% 
  mutate(strategy_count = factor(strategy_count, levels = 0:4)) %>% 
