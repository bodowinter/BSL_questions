...
CA_p
ggsave(plot = CA_p,
       file = '../figures/pdf/constructed_action.pdf',
       width = 3.5, height = 6)
ggsave(plot = CA_p,
       file = '../figures/png/constructed_action.png',
       width = 3.5, height = 6)
```
...
ggsave(plot = semantic_p,
       file = '../figures/pdf/semantic.pdf',
       width = 7.5, height = 4.5)
ggsave(plot = semantic_p,
       file = '../figures/png/semantic.png',
       width = 7.5, height = 4.5)
```
...
ggsave(plot = fig2,
       filename = '../figures/pdf/figure2.pdf',
       width = 13, height = 3.5)
ggsave(plot = fig2,
       filename = '../figures/png/figure2.png',
       width = 13, height = 3.5)
```
...
ggsave(plot = fig3,
       ...
       width = 13, height = 3.5)
ggsave(plot = fig3,
       filename = '../figures/png/figure3.png',
       width = 13, height = 3.5)
```
...
Transform the variable into one that has "single", "multiple" or "no" head movements. First, define vectors for that:
...
single <- c('up',
            ...
            'null, up', # with null
            'null, down', # with null
            'null, squint', # with null
            'up, null', # with null
            'down, null', # with null
            'null, up, null', # with null
            'squint, null', # with null
            'null, down, null', # with null
            'null, squint, null') # with null
...
Then, create a new variable that has a transformed variable:

```{r}
df$eye_clean <- NA

df <- mutate(df,
             eye_clean = ifelse(eye_n_brow %in% single, 'single', eye_clean),
             eye_clean = ifelse(eye_n_brow %in% multiple, 'multiple', eye_clean),
             eye_clean = ifelse(eye_n_brow %in% none, 'none', eye_clean))
```

Check:

```{r}
df %>% count(eye_n_brow, eye_clean) %>% 
  print(n = Inf)
```

Make a table for the cleaned and simplified eye brow variable:

```{r}
df %>%
  count(eye_clean, sort = TRUE) %>%
  prop_table() %>% 
  print(n = Inf)
```

Then, transform the eye brow variable into one that is either 'up', 'down' or 'neither'. We convert cases  like 'up, null' or 'null, down 'to 'up' and 'down' respectively etc. This is to look at the specific movements, and whether they differ between polar and content.

```{r}
singles_tab <- df %>%
  filter(eye_clean == 'single') %>% 
  mutate(eye_n_brow = str_replace_all(eye_n_brow, '(, null)|(null, )', '')) %>% 
  count(semantic, eye_n_brow) %>%
  filter(eye_n_brow %in% c('up', 'down', 'squint')) %>%
  group_by(semantic) %>%
  prop_table()
...
```{r}
eyes_tab <- df %>% 
  count(semantic, eye_clean) %>% 
  filter(!is.na(eye_clean)) %>% # get rid of NA's
  group_by(semantic) %>% 
  prop_table()
...
eyes_p <- eyes_tab %>% 
  mutate(semantic = ifelse(semantic == 'Content',
                           'Content\nN = 137',
                           'Polar\nN = 412')) %>% 
  ggplot(aes(x = semantic, y = n, fill = eye_clean)) +
  geom_col(position = 'fill', width = 0.65, col = 'black') +
  coord_flip() +
  scale_fill_viridis_d(guide = guide_legend(reverse = TRUE))
...
Also a plot of singles only:

```{r, fig.width = 8, fig.height = 6}
# Aesthetics, geoms, and coord_flip():
  
singles_p <- singles_tab %>% 
  mutate(semantic = ifelse(semantic == 'Content',
                           'Content\nN = 106',
                           'Polar\nN = 287')) %>% 
...
Put into double plot with eyes:

```{r}
eyes_p <- eyes_p +
  ggtitle('(a) Eyebrow movements')
singles_p <- singles_p +
  ggtitle('(b) Questions with only one movement')

# Combine:

eyes_double_p <- eyes_p +
  plot_spacer() +
  singles_p +
  plot_layout(widths = c(20, 0.5, 20), nrow = 1)
...
ggsave(plot = eyes_double_p,
       filename = '../figures/pdf/eyes_double_plot.pdf',
       width = 13, height = 3.5)
ggsave(plot = eyes_double_p,
       filename = '../figures/png/eyes_double_plot.png',
       width = 13, height = 3.5)
```

Clean the head movement column:

```{r}
df <- mutate(df,
             head = str_to_lower(head),
             head = str_replace_all(head, 'hs1', 'hs'),
             head = str_replace_all(head, 'nod1', 'nod'))
```

First, check whether there are single or multiple head movements. For this we first need to simplify the head variable:

```{r}
# Make conversions:

df <- mutate(df,
             head = ifelse(head == 'nod, null', 'nod', head),
             head = ifelse(head == 'null, nod', 'nod', head),
             head = ifelse(head == 'hs, null', 'hs', head),
             head = ifelse(head == 'nod, null, nod', 'nod', head),
             head = ifelse(head == 'null, nod, null', 'nod', head),
             head = ifelse(head == 'nod, nod', 'nod', head),
             head = ifelse(head == 'backward, null', 'backward', head),
             head = ifelse(head == 'forward, null', 'forward', head),
             head = ifelse(head == 'null, down, null', 'down', head),
             head = ifelse(head == 'nod, nod, null', 'nod', head),
             head = ifelse(head == 'null, forward, null', 'forward', head),
             head = ifelse(head == 'null, hs', 'hs', head),
             head = ifelse(head == 'null, nod, nod', 'nod', head),
             head = ifelse(head == 'null, sideward', 'sideward', head),
             head = ifelse(head == 'down, null', 'down', head),
             head = ifelse(head == 'nod, null, nod, null, nod', 'nod', head),
             head = ifelse(head == 'null, down', 'down', head),
             head = ifelse(head == 'null, nod?', 'nod', head),
             head = ifelse(head == 'null, null', 'null', head),
             head = ifelse(head == 'null, sideward, null', 'sideward', head),
             head = ifelse(head == 'up, null', 'up', head),
             head = ifelse(head == 'not clear', NA, head),
             head = ifelse(head == 'null', 'none', head),
             head = ifelse(head == 'null, forward', 'forward', head),
             head = ifelse(head == 'hs', 'headshake', head))

# Vector of singles:

singles <- c('nod', 'forward', 'headshake',
             'sideward', 'down', 'up', 'backward')

# Create the single versus multiple versus none variable

df <- mutate(df,
             head_type = ifelse(head %in% singles, 'single', head),
             head_type = ifelse(head_type == 'single' | head_type == 'none', head_type, 'multiple'))
```
...
```{r}
head_type_tab <- df %>%
  count(semantic, head_type) %>%
  filter(!is.na(head_type)) %>% 
  group_by(semantic) %>%
  prop_table()
...
head_type_p <- head_type_tab %>% 
  mutate(semantic = ifelse(semantic == 'Content',
                           'Content\nN = 139',
                           'Polar\nN = 416')) %>% 
  ggplot(aes(x = semantic, y = n, fill = head_type)) +
  geom_col(position = 'fill', width = 0.65, col = 'black') +
  coord_flip() +
  scale_fill_viridis_d(guide = guide_legend(reverse = TRUE))
...
There are very few up and backward. Let's exclude them for plotting as these will be impossible to see anyway:
...
head_p
ggsave(plot = head_p + theme(plot.margin = margin(r = 20)) + ggtitle("Head movements"),
       filename = '../figures/pdf/head_movements_only.pdf',
       width = 7, height = 4)
ggsave(plot = head_p + theme(plot.margin = margin(r = 20)) + ggtitle("Head movements"),
       filename = '../figures/png/head_movements_only.png',
       width = 7, height = 4)
```
...
ggsave(plot = head_double_p,
       filename = '../figures/pdf/head_double_plot.pdf',
       width = 13, height = 3.5)
ggsave(plot = head_double_p,
       filename = '../figures/png/head_double_plot.png',
       width = 13, height = 3.5)
```
...
strategy_p
ggsave(plot = strategy_p, file = '../figures/pdf/strategy_count.pdf',
       width = 8, height = 5.5)
ggsave(plot = strategy_p, file = '../figures/png/strategy_count.png',
       width = 8, height = 5.5)
```
...
```{r}
position_tab <- df %>%
  filter(manual %in% c('Single', 'Multiple')) %>% 
...
position_p
ggsave(plot = position_p, file = '../figures/pdf/positions.pdf',
       width = 10, height = 6)
ggsave(plot = position_p, file = '../figures/png/positions.png',
       width = 10, height = 6)
```
...
response_p <- response_p +
  ggtitle('(b) Requested response')

# Combine:

response_double_p <- question_p +
  plot_spacer() +
  response_p +
  plot_layout(widths = c(20, 0.5, 20), nrow = 1)

ggsave(plot = response_double_p,
       filename = '../figures/pdf/question_response.pdf',
       width = 13, height = 3.5)
ggsave(plot = response_double_p,
       filename = '../figures/png/question_response.png',
       width = 13, height = 3.5)
```
